...
class _DeveloperScreenState extends State<DeveloperScreen> {
  ...
  Future<void> _authenticate() async {
    ...
    if (PinSecurityService.verifyMasterPin(_prefs!, pin)) {
      ...
      setState(() { _authenticated = true; _authError = null; });
    } else {
  ...
  Future<void> _saveMasterPin() async {
    ...
    if (newPin.length < 8) {
      setState(() { _saveError = 'لازم يكون 8 أحرف على الأقل'; _saveSuccess = null; });
      return;
    }
    if (newPin != confirm) {
      setState(() { _saveError = 'كلمتا المرور مش متطابقتين'; _saveSuccess = null; });
      return;
    }
  ...
  Future<void> _resetUserPin() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('رقم المستخدم الجديد: $newPin  (سيُحذف من هنا بعد 10 ثواني)'),
  ...
  Widget _buildAuthGate() => Column(
    ...
    children: [
      ...
      Icon(Icons.shield_moon_rounded, size: 64, color: Theme.of(context).colorScheme.primary),
      const SizedBox(height: 20),
      const Text('وضع المطور', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
      ...
      if (_hasMaster) ...[
        TextField(
          ...
          decoration: InputDecoration(
            ...
            suffixIcon: IconButton(
              icon: Icon(_obscureAuth ? Icons.visibility_off : Icons.visibility),
        ...
        if (_authError != null) ...[
          ...
          Text(_authError!, style: const TextStyle(color: Colors.red, fontSize: 13), textAlign: TextAlign.center),
        ],
        const SizedBox(height: 20),
        SizedBox(
      ...
      _card(child: Column(
        ...
        children: [
          ...
          if (_saveSuccess != null) ...[
            const SizedBox(height: 8),
            Text(_saveSuccess!, style: const TextStyle(color: Colors.green, fontSize: 12)),
          ],
          const SizedBox(height: 16),
          SizedBox(
            width: double.infinity,
        ...
        children: [
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              label: const Text('إعادة تعيين', style: TextStyle(color: Colors.orange)),
  ...
  Widget _infoRow(String k, String v) => Padding(
    ...
    child: Row(
      ...
      children: [
        Text(k, style: TextStyle(fontSize: 13, color: Theme.of(context).colorScheme.onSurfaceVariant)),
